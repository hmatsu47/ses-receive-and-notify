...
| ID | 要件 | 由来 |
|----|------|------|
| NFR3.1 | Lambda実行ロールのIAM権限は最小権限とする：1st S3の get、2nd S3の put（および必要なら get）、CloudWatch Logs の書き込み、Slack Webhook URL等を格納する Secrets Manager/SSM Parameter の該当パスの read に限定。ワイルドカード権限（`s3:*`、`*` リソース）を用いない。 | NFR3, Mandated(最小権限) |
