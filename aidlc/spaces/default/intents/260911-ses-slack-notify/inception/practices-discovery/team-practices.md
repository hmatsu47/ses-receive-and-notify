# チーム作業実践 — SES受信メールのSlack通知ツール

> 本ドキュメントは Practices Discovery ステージの最終統合結果です。リードのドラフトに、
> 品質・開発・DevSecOps の各支援貢献とヒアリング（Q1〜Q6）の確定事項を反映しています。
> 単独開発・単独運用の小規模 AWS サーバーレスツール（SES受信 → S3 → Lambdaデコード →
> 2nd S3 → Slack通知）に合わせて特化しています。
